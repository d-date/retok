...
./retok --dirs ~/.claude/projects   # スキャン対象を指定
```

デフォルトで `~/.claude/projects` と `~/.claude-personal/projects` を走査します。
同一メッセージIDはグローバルに重複排除されるため、両ディレクトリにミラーされたファイルがあっても二重計上されません。
