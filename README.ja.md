...
- **巨大コンテキスト** — 120k tokens 超、かつプロンプト数3件以上のセッションを列挙。タスク間 `/clear` の徹底を推奨
- **探索の委譲不足** — メインスレッドの Read/Grep/Glob 比率が高くサブエージェント利用が少ない場合
- **再試行ループ** — 同一Bashコマンドをセッション内で5回以上実行
- **中断多発** — `[Request interrupted by user]` がプロンプト数の12%超（対象期間のプロンプト数が30件を超えた場合のみ判定） → 指示の明確化・Plan Mode活用を推奨
- **高価格モデルでの小粒セッション** — 一問一答レベルの用途でOpus/Fableを使用しているセッションが対象期間で10件以上
- **高価格帯モデルへの集中** — 価格中央値より高いモデル（現状 Fable/Mythos）1つが総コストの90%以上を占め、かつ総コストが$50以上。安価なモデルへの軽作業の委譲を推奨
